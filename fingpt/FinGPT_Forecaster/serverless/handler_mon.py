...
ADAPTER_ID_FRI_FRI = os.environ.get("ADAPTER_PATH_FRI_FRI", ADAPTER_ID)
ADAPTER_ID_MON_FRI = os.environ.get("ADAPTER_PATH_MON_FRI", ADAPTER_ID)
DEFAULT_WEEK_MODE = os.environ.get("DEFAULT_WEEK_MODE", "mon_fri_preopen")
...
def _resolve_adapter_id(week_mode: str, adapter_override: Optional[str] = None) -> str:
    if adapter_override:
        return adapter_override
    if week_mode == "mon_fri_preopen":
        return ADAPTER_ID_MON_FRI
    return ADAPTER_ID_FRI_FRI


def load_model(week_mode: str = "mon_fri_preopen", adapter_override: Optional[str] = None):
    """Load base model once and attach adapter for selected horizon."""
    global model, tokenizer, finnhub_client, base_model, current_adapter_id

    target_adapter = _resolve_adapter_id(week_mode, adapter_override)

    with model_lock:
        if model is not None and current_adapter_id == target_adapter:
            return
        ...
        try:
            ...
            print(f"Using Adapter ID: {target_adapter}", flush=True)
            ...
            # Load/swap LoRA adapter for selected horizon
            if "/" in target_adapter and not os.path.exists(target_adapter):
                model = PeftModel.from_pretrained(base_model, target_adapter, token=hf_token)
            else:
                model = PeftModel.from_pretrained(base_model, target_adapter)
            model = model.eval()
            current_adapter_id = target_adapter
...
def _most_recent_monday_on_or_before(curday: str) -> datetime:
    dt = datetime.strptime(curday, "%Y-%m-%d")
    return dt - timedelta(days=dt.weekday())


def _build_history_periods(curday: str, n_weeks: int, week_mode: str) -> List[Tuple[str, str]]:
    if week_mode == "mon_fri_preopen":
        anchor_monday = _most_recent_monday_on_or_before(curday)
        periods = []
        for i in range(n_weeks, 0, -1):
            week_start = anchor_monday - timedelta(days=7 * i)
            week_end = week_start + timedelta(days=4)
            periods.append((week_start.strftime("%Y-%m-%d"), week_end.strftime("%Y-%m-%d")))
        return periods

    # Legacy fallback: rolling 7-day blocks
    steps = [n_weeks_before(curday, n) for n in range(n_weeks + 1)][::-1]
    return [(steps[i], steps[i + 1]) for i in range(len(steps) - 1)]


def _get_prediction_period(curday: str, week_mode: str) -> Tuple[str, str]:
    if week_mode == "mon_fri_preopen":
        start = _most_recent_monday_on_or_before(curday)
        end = start + timedelta(days=4)
        return start.strftime("%Y-%m-%d"), end.strftime("%Y-%m-%d")
    return curday, n_weeks_before(curday, -1)


def _get_news_window(start_dt: datetime, end_dt: datetime, week_mode: str) -> Tuple[str, str]:
    if week_mode == "mon_fri_preopen":
        # Include weekend before the trading week start.
        days_since_saturday = (start_dt.weekday() - 5) % 7
        from_dt = start_dt - timedelta(days=days_since_saturday)
        return from_dt.strftime("%Y-%m-%d"), end_dt.strftime("%Y-%m-%d")
    return start_dt.strftime("%Y-%m-%d"), end_dt.strftime("%Y-%m-%d")


def get_stock_data(stock_symbol: str, periods: List[Tuple[str, str]], week_mode: str) -> pd.DataFrame:
    """Fetch stock price data for explicit periods."""
    if not periods:
        return pd.DataFrame(columns=["Start Date", "End Date", "Start Price", "End Price"])

    dl_start = min(p[0] for p in periods)
    dl_end_dt = datetime.strptime(max(p[1] for p in periods), "%Y-%m-%d") + timedelta(days=1)
    stock_data = yf.download(stock_symbol, dl_start, dl_end_dt.strftime("%Y-%m-%d"), progress=False)
    if len(stock_data) == 0:
    ...
    start_col = "Open" if week_mode == "mon_fri_preopen" and "Open" in stock_data.columns else "Close"
    end_col = "Close" if "Close" in stock_data.columns else start_col

    rows = []
    for start_str, end_str in periods:
        start_dt = datetime.strptime(start_str, "%Y-%m-%d")
        end_dt = datetime.strptime(end_str, "%Y-%m-%d")
        period_df = stock_data.loc[(stock_data.index >= start_dt) & (stock_data.index <= end_dt)]
        period_df = period_df.dropna(subset=[start_col, end_col])
        if period_df.empty:
            continue

        start_idx = period_df.index[0]
        end_idx = period_df.index[-1]
        start_price = float(period_df.iloc[0][start_col])
        end_price = float(period_df.iloc[-1][end_col])
        if start_price == 0:
            continue

        rows.append({
            "Start Date": start_idx.to_pydatetime(),
            "End Date": end_idx.to_pydatetime(),
...
def get_news(symbol, data, week_mode: str = "mon_fri_preopen"):
    ...
    for _, row in data.iterrows():
        start_date, end_date = _get_news_window(row["Start Date"], row["End Date"], week_mode)
...
def get_preopen_weekend_news(symbol: str, curday: str, k: int = 5) -> list:
    """Fetch most relevant weekend news available before Monday open."""
    if finnhub_client is None:
        return []
    cur_dt = datetime.strptime(curday, "%Y-%m-%d")
    days_since_saturday = (cur_dt.weekday() - 5) % 7
    from_dt = cur_dt - timedelta(days=days_since_saturday)
    ...
    try:
        ...
        ranked = rank_news_by_relevance(processed, curday)
        return ranked[:k]
    except Exception as e:
        print(f"Weekend news fetch error: {e}", flush=True)
        return []
...
class PromptBuilder:
    ...
    def add_weekend_news(self):
        if self.context.week_mode != "mon_fri_preopen":
            return self
        formatted = []
        for n in self.context.weekend_news[:5]:
            headline = (n.get("headline") or "").strip()
            summary = (n.get("summary") or "").strip()
            if headline and summary:
                formatted.append(f"[Headline]: {headline}\n[Summary]: {summary}\n")
        if formatted:
            self.parts.append("\n[WEEKEND NEWS - PRE-MARKET]:\n" + "\n".join(formatted))
        else:
            self.parts.append("\n[WEEKEND NEWS - PRE-MARKET]:\nNo weekend news reported.")
        return self

    def add_instruction(self):
        period_start, period_end = self.context.prediction_period
        ...
        if self.context.week_mode == "mon_fri_preopen":
            instruction += " In this mode, [WEEKEND NEWS - PRE-MARKET] is the latest information available before market open."
        self.parts.append("\n" + instruction)
        return self
...
def construct_prompt(ticker, curday, n_weeks, use_basics=True, use_quant_signals=True, week_mode="mon_fri_preopen"):
    """Build the full prompt for the model."""
    periods = _build_history_periods(curday, n_weeks, week_mode)
    data = get_stock_data(ticker, periods, week_mode)
    ...
    weekend_news = get_preopen_weekend_news(ticker, curday) if week_mode == "mon_fri_preopen" else []
    prediction_period = _get_prediction_period(curday, week_mode)
...
def predict(
    ...
    n_weeks=3,
    use_basics=True,
    use_quant_signals=True,
    week_mode="mon_fri_preopen",
    adapter_override=None
):
    """Generate stock prediction."""
    load_model(week_mode=week_mode, adapter_override=adapter_override)  # Ensure model is loaded
    ...
    prompt = construct_prompt(
        ticker, prediction_date, n_weeks, use_basics, use_quant_signals, week_mode=week_mode
    )
...
def handler(event):
    ...
    try:
        ...
        week_mode = input_data.get("week_mode", DEFAULT_WEEK_MODE)
        if week_mode not in {"fri_fri", "mon_fri_preopen"}:
            return {"error": "Invalid week_mode. Use 'fri_fri' or 'mon_fri_preopen'."}
        adapter_override = input_data.get("adapter_path")
        ...
        response = {
            ...
            "week_mode": week_mode,
            "adapter": _resolve_adapter_id(week_mode, adapter_override),
        }
