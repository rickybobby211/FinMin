...
def extract_preopen_weekend_news(news_items: list, start_date: str) -> list:
    """Extract pre-open weekend news right before the target week start."""
    start_dt = pd.to_datetime(start_date)
    weekend_start = start_dt - timedelta(days=((start_dt.weekday() - 5) % 7))
    selected = []
    for item in news_items:
        ...
        if news_dt is None:
            continue
        if weekend_start <= news_dt < start_dt:
...
def get_all_prompts(
    ...
    if with_basics:
        if week_mode == "mon_fri_preopen":
            csv_file = list(Path(data_dir).glob(f"{symbol}_*_*_monfri.csv"))
        else:
            csv_file = list(Path(data_dir).glob(f"{symbol}_*_*.csv"))
            csv_file = [f for f in csv_file if "_monfri" not in str(f)]
        csv_file = [f for f in csv_file if 'nobasics' not in str(f) and 'gpt-4' not in str(f)]
    else:
        if week_mode == "mon_fri_preopen":
            csv_file = list(Path(data_dir).glob(f"{symbol}_*_*_monfri_nobasics.csv"))
        else:
            csv_file = list(Path(data_dir).glob(f"{symbol}_*_*_nobasics.csv"))
            csv_file = [f for f in csv_file if "_monfri_" not in str(f)]
    ...
    target_set = set()
    if target_indices:
        target_set = {int(i) for i in target_indices if 0 <= int(i) < len(df)}

    if target_set:
        max_needed_idx = max(target_set)
        preprocess_rows = max_needed_idx + 1
        print(f"    Pre-processing {preprocess_rows}/{len(df)} rows for {symbol} (targeted retry)...")
    else:
        preprocess_rows = len(df)
        print(f"    Pre-processing {len(df)} rows for {symbol}...")

    processed_rows = []
    
    for row_idx, row in df.iloc[:preprocess_rows].iterrows():
    ...
    def _build_single_prompt_worker(p_row):
        ...
        if week_mode == "mon_fri_preopen":
            weekend_news = extract_preopen_weekend_news(p_row["news_raw"], p_row["start_date"])
            sampled_weekend = sample_news(
                weekend_news,
                min(5, len(weekend_news)),
                strategy=news_strategy,
                end_date=p_row["start_date"],
                symbol=symbol,
                client=client,
                model=model,
            ...
            weekend_formatted = format_news_items(sampled_weekend)
            if weekend_formatted:
                prompt_str += "\\n[WEEKEND NEWS - PRE-MARKET]:\\n" + "\\n".join(weekend_formatted)
        ...
        if week_mode == "mon_fri_preopen":
            full_prompt += f"Then let's assume your prediction for the coming trading week ({p_row['start_date']} pre-market to {p_row['end_date']} close) is {prediction}. Pay special attention to [WEEKEND NEWS - PRE-MARKET], because this information is available before Monday open. "
        else:
            full_prompt += f"Then let's assume your prediction for next week ({p_row['start_date']} to {p_row['end_date']}) is {prediction}. "
        full_prompt += f"Provide a summary analysis to support your prediction. Before writing the analysis, cross-reference the [QUANT SIGNALS] with the [NEWS]. If the prediction is {prediction} but the technicals show a conflict (e.g., predicting 'Up' when Trend is 'Strong Downtrend'), explain this divergence logically (e.g., as a 'trend reversal' or 'short squeeze') rather than ignoring the data. The prediction result need to be inferred from your analysis at the end, and thus not appearing as a foundational factor of your analysis."

        return (curr_idx, full_prompt.strip(), prediction, p_row['start_date'], p_row['end_date'])

    all_prompts = []
    rows_for_prompt_build = (
        [row for row in processed_rows if row["idx"] in target_set]
        if target_set else processed_rows
    )
    ...
    if parallel > 1 and news_strategy == 'llm':
        with ThreadPoolExecutor(max_workers=parallel) as executor:
            # Create futures
            futures = [executor.submit(_build_single_prompt_worker, row) for row in rows_for_prompt_build]
        ...
        for row in tqdm(rows_for_prompt_build, desc=f"    {symbol} Prompts", unit="gen"):
            res = _build_single_prompt_worker(row)
            if res:
                all_prompts.append(res)

    # Sort by row index to keep stable index mapping for resume logic.
...
def query_llm(
    ...
    csv_file = f'{data_dir}/{symbol}{mode_suffix}{suffix}_gpt-4.csv'
    ...
    if os.path.exists(csv_file):
        try:
            ...
            if 'index' in existing_df.columns:
                index_values = pd.to_numeric(existing_df['index'], errors='coerce').dropna().astype(int).tolist()
                done_indices = set(index_values)
            else:
    ...
    prompts_with_index = [
        (idx, prompt, prediction, start_date, end_date)
        for (idx, prompt, prediction, start_date, end_date) in prompts
...
def main():
    def parse_only_indices(raw_value: str) -> dict:
        """
        Parse format:
          "AMZN:32;TSM:48,49,50"
        """
        mapping = {}
        if not raw_value:
            return mapping
        ...
        for chunk in chunks:
            if ":" not in chunk:
                continue
            symbol_key, index_part = chunk.split(":", 1)
            symbol_key = symbol_key.strip().upper()
            indices = set()
            for token in index_part.split(","):
                token = token.strip()
                if not token:
                    continue
                try:
    ...
    parser.add_argument('--week_mode', type=str, default='mon_fri_preopen',
                        ...
                        help='Week window mode: fri_fri (legacy) or mon_fri_preopen')
    parser.add_argument(
        '--only_indices',
        type=str,
        default='',
        help='Optional targeted retry format: "AMZN:32;TSM:48,49,50"',
    )
    args = parser.parse_args()
    only_indices_map = parse_only_indices(args.only_indices)
    ...
    for symbol in sorted(symbols):
        print(f"\\nProcessing {symbol}...")
        target_indices = only_indices_map.get(symbol)
        query_llm(
