...
def order_labels_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    """
    Keep deterministic chronological order before train/test split.
    """
    ordered = df.copy()

    if 'start_date' in ordered.columns:
        ordered['_sort_start_date'] = pd.to_datetime(ordered['start_date'], errors='coerce')
        if 'index' in ordered.columns:
            ordered['_sort_index'] = pd.to_numeric(ordered['index'], errors='coerce')
            ordered = ordered.sort_values(
                by=['_sort_start_date', '_sort_index'],
                kind='mergesort',
                na_position='last'
            ).drop(columns=['_sort_start_date', '_sort_index'])
        else:
            ordered = ordered.sort_values(
                by=['_sort_start_date'],
                kind='mergesort',
...
def gpt4_to_llama(
    ...
    for i, row in df.iterrows():
        ...
        res = re.search(
            r"Then let's assume your prediction for (?:next week|the coming trading week) \((.*)\) is ((?:up|down) by .*%)\.",
            prompt
        )
        ...
        prompt = re.sub(
            r"Then let's assume your prediction for (?:next week|the coming trading week) \((.*)\) is (?:up|down) by (?:.*?)%\.[\s\S]*?Provide a summary analysis to support your prediction[\s\S]*?The prediction result need to be inferred from your analysis at the end, and thus not appearing as a foundational factor of your analysis.",
...
def create_dataset(
    ...
    if symbols is None:
        suffix = "" if with_basics else "_nobasics"
        mode_suffix = "_monfri" if week_mode == "mon_fri_preopen" else ""
        csv_files = list(Path(data_dir).glob(f"*{mode_suffix}{suffix}_gpt-4.csv"))
        symbols = [f.stem.replace(mode_suffix + suffix + "_gpt-4", "") for f in csv_files]
    ...
    for symbol in symbols:
        data_dict = gpt4_to_llama(symbol, data_dir, with_basics, format_type, week_mode)
...
def main():
    ...
    parser.add_argument('--week_mode', type=str, default='mon_fri_preopen',
                        ...
                        help='Week window mode used when labels were generated')
